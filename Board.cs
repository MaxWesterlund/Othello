...
public class Board {
    ...
    void PointsCounter() {
        ...
        int blackMargin = Raylib.MeasureText(blackPoints.ToString(), 40);
        Raylib.DrawText(blackPoints.ToString(), (int)(720f - blackMargin/2f), 104, 40, Color.BLACK);
        Raylib.DrawCircle(720, 194, 32, Color.WHITE);
        int whiteMargin = Raylib.MeasureText(whitePoints.ToString(), 40);
        Raylib.DrawText(whitePoints.ToString(), (int)(720f - whiteMargin/2f), 234, 40, Color.WHITE);
        
        if (blackPoints + whitePoints == 64) {
            if (blackPoints > whitePoints) {
                DrawTurn(Color.BLACK, "Black Wins!");
                gameOver = true;
                return;
            }
            else {
                DrawTurn(Color.WHITE, "White Wins!");
                gameOver = true;
                return;
            }
        }
